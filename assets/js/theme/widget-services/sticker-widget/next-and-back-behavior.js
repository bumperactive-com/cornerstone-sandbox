...
export default function nextAndBackBehavior(commonWidgetHTML, roundAttributeValue, roundToTwoDecimals, stickerWidgetHTML, addToCart, hiddenElements) {
    ...
    var radioProdOptions = document.querySelectorAll('[data-product-attribute="set-radio"]');
    ...
    const stickerFinishSection = document.getElementById("stickerFinishSection");
                    ...
                    switch (value) { 
                      case "next": 
                        ...
                        //Sticker Finish radio buttons
                            var finishRadioButtons = document.getElementById("stickerFinishSection").getElementsByClassName("form-radio");
                           ...
                           var urlWithhNumber = url.split('/');
                           var titleList;
                           //The code below checks the storefront API if a $75 item is in cart already.
                        ...
                        window.location.href = `${siteURL}/${newString}-minimum-${maxSequenceNumber}/?width=${hiddenWidth}&height=${hiddenHeight}&quantity=${hiddenQuantity}&finish=${finish}&originalURL=${urlWithhNumber[3]}`;
                        });
                     }
                        var thatsOkButton = document.getElementsByClassName("swal-button swal-button--next")[0];
    ...
    commonWidgetHTML.pageElements.backButton3.onclick = function () { 
        specsDiv.style.display = "none";
        stickerWidgetHTML.imageDecision.style.display = "block";
        document.getElementById("imageDecision").style.display = "block";
        nextButtonText.style.display = "block";
